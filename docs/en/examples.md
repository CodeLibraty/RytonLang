...
- Graphical/User interface(UI) on RuVix:
```ryton
module import {
    std.RuVix.App
    std.RuVix.Effects
...
app.run()
```
Stylish blue text editor with effects and animations

- Creating DSL languages
```ryton
module import {
    std.DSL
}
...
```
Simple DSL for working with colors
...
```
Simple program for displaying time in console with ascii and color

- Full-fledged shell of command line:
```ryton
module import {
    std.Terminal[Terminal]
    std.Files
...
```
This example demonstrates a high-quality and easy-to-write command shell for posix systems

- Example of using Zig integration:
1. executes code in place:
```ryton
trash_cleaner = true
...
```
In this example we use Zig integration to cause a simple Zig code to be compiled and run
2. creating a Zig module:
```ryton
trash_cleaner = true
func Main {
    print("Hello From Ryton")
...
```
In this example we use Zig integration to create a module in Zig and call a function from it